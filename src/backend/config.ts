...
import { collectLeafPaths, CONFIG_LEGACY_ENV_ALIASES, toEnvKey } from '../types/configEnv'
...
const LEGACY_ENV_ALIASES = CONFIG_LEGACY_ENV_ALIASES as Partial<Record<EnvConfigPath, readonly string[]>>
...
const socketHandler = (socket: net.Server, resolve: (value: boolean) => void, reject: (reason: Error) => void) => {
  ...
  socket.addListener('error', (err: Error) => {
    socket.close()
    if ((err as NodeJS.ErrnoException).code === 'EADDRINUSE') resolve(true)
    else reject(err)
  })
}
...
const getEnvLockedPaths = (env: EnvConfigInput): EnvConfigPath[] => ENV_CONFIG_PATHS
...
export const createLiveConfig = async ({
  ...
  if (guiConfig) {
    const filteredGuiConfig = stripEnvLockedFields(stripAutoManagedNodeFields(guiConfig), envLockedPathSet)
    if (filteredGuiConfig) applyRuntimePatch(config, filteredGuiConfig)
    if (!isEnvIpLocked) config.node.ip = ip
    if (!isEnvHostnameLocked && (config.node.hostname.trim().length === 0 || config.node.hostname === DEFAULT_CONFIG.node.hostname)) {
      config.node.hostname = ip
    }
